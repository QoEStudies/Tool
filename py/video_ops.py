...
def drop_frames(input_file, output_file, drop_percentage):
    # Load the video clip
    clip = VideoFileClip(input_file)
    ...
    selected_clip = selected_clip.set_duration(len(selected_frames) / clip.fps)
    selected_clip = selected_clip.set_make_frame(lambda t: selected_frames[int(t * clip.fps)])

    # Write the final video to the output path
    selected_clip.write_videofile(output_file)


# drop_frames('input.mp4', 'output.mp4', 0.5)
